...
void setup() {
  ...
  Wire.write(0x6B);                  // Access the register 6B
  Wire.write(0x00);                  // Reset
  Wire.endTransmission(true);        // end the transmission
  s1.attach(2);
  s2.attach(3);
  s3.attach(4);
  calculate_IMU_error();            //Calculate the errors
  delay(20);
}
void loop() {
  ...
  currentTime = millis();            // Current actual time read
  ...
  // Currently the raw values are in degrees per seconds, so we need to multiply by seconds(s) to get the angle in degrees
  ...
  // Control servo motors of the gimbal
  double x = 90-roll;
  double y = 90-pitch;
  double z = 90-yaw;
  s1.write(x+2);
